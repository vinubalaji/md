...
window.$t = window.Tiggr = (function() {
	...
	// variants are :
	// 1) Tiggr(String ... ids);
	// 2) Tiggr(String ... ids, Object context);
	var tiggrFunction = function() {
		var id;
		var context;	
...
(function($t, $) {
					...
					if(elem.attr("_idx")==undefined){
						...
						//clone tiggzi JS object 					
							if(Tiggr(name)!=undefined && Tiggr(name).getOptions!=undefined){								
								if(elem.attr("tiggrclass")!=undefined){
									if(elem.attr("tiggrclass")=="datepicker"){				
										options=Tiggr(name).getOptions();
										return new Tiggr.TiggrMobileDatePickerComponent(elem.attr("id"), options);
									}else if(elem.attr("tiggrclass")=="carousel"){
		...
		__refreshComponent: function() {		
			if(this.componentsForRefresh!=undefined){
				$.each(this.componentsForRefresh,function(i, val){ 					
					if(Tiggr(val)!=undefined && Tiggr(val).refresh!=undefined){
						Tiggr(val).refresh();
					}	
				clonedElementsSelector="[name='"+val+"'][_idx]";
				
				$.each($(clonedElementsSelector),function(){ 
							if($(this).attr("tiggrclass")!=undefined){
									if($(this).attr("tiggrclass")=="carousel"){
	...
	$t.TiggrMobileCarouselComponent = $t.createClass($t.TiggrWrapper, {
		init:function(componentId,options){
				//this.constructor.$super.init(componentId,options);				
